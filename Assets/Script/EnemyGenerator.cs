...
public class EnemyGenerator : MonoBehaviour
{
    ...
    //Enemy1,3出現数の上限
    private int[] EnemyLimits = { 7, 2, 1 };
    // 時間計測用の変数(Enemy1,2,3,LevelUp)
    ...
    void Update()
    {
        ...
        if (Enemy1counts <= EnemyLimits[0] && this.delta[0] > Seconds[0])
        {
            this.delta[0] = 0;
        ...
        //画面内に敵2が1体以下の状態で､5秒以上経過したとき(Level==1の場合)
            if (Enemy2counts < EnemyLimits[1] && this.delta[1] > Seconds[1])
        {
            this.delta[1] = 0;
            ...
            {
                //生成場所のランダム指定
                int Enemy2pos = Random.Range(0, EnemyLimits[1]);
                //生成場所の指定
        ...
        if (Enemy3counts <= EnemyLimits[2] && this.delta[2] > Seconds[2])
        {
            this.delta[2] = 0;
        ...
        if(delta[3] >= 30.0f)
        {
            ...
            if(this.Level >= 1.25 && EnemyLimits[1] != 3)
            {
                EnemyLimits[1] = 3;
            }
            //時間リセット
